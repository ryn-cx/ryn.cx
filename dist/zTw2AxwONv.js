...
const placeholders = document.querySelectorAll(".winbox-placeholder");
	const winboxes = new Map();

	function openWinbox(placeholder) {
		const content = placeholder.querySelector(".winbox-content");
		const rect = placeholder.getBoundingClientRect();
		const mount = content.cloneNode(true);
		mount.removeAttribute("hidden");
		const wb = new WinBox(placeholder.dataset.title, {
			...
			mount: mount,
			onclose: () => {
				winboxes.delete(placeholder);
			},
		});
		winboxes.set(placeholder, wb);
	}

	placeholders.forEach(openWinbox);

	const title = document.querySelector("h1");
	if (title && placeholders.length) {
		title.style.cursor = "pointer";
		title.addEventListener("click", () => {
			placeholders.forEach((placeholder) => {
				if (!winboxes.has(placeholder)) {
					openWinbox(placeholder);
				}
			});
		});
	}